...
type DocForLearning struct {
	ID      int64
	Title   string
	Content string
	TopicID string // from weak/gold query
}

func ListDocsForWeakLabel(ctx context.Context, conn *pgx.Conn, limit int) ([]DocForLearning, error) {
	...
	var out []DocForLearning
	for rows.Next() {
		var r DocForLearning
...
// Training set ưu tiên gold, fallback weak (confidence>=minConf)
func ListTrainingSet(ctx context.Context, conn *pgx.Conn, minWeakConf float32, limit int) ([]DocForLearning, error) {
