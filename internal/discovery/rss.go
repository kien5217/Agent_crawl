...
func NewRSSDiscovery(cfg config.Config, sources config.SourcesFile) *RSSDiscovery {
	return &RSSDiscovery{
		cfg:     cfg,
		sources: sources,
...
func (d *RSSDiscovery) Enqueue(ctx context.Context, conn *pgx.Conn) (int, error) {
	fp := gofeed.NewParser()
	enqueued := 0
	...
	for _, s := range d.sources.Sources {
		...
		for i, item := range feed.Items {
			...
			inserted, err := db.EnqueueURL(ctx, conn, "cve", s.ID, norm, domain, 10)
			if err != nil {
