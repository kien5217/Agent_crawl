...
2) Cách chạy:
   ...
   - Chạy schedule để enqueue các bài viết từ RSS và sitemap: go run main.go schedule  --config ./config/config.yaml
     (Lưu ý: nếu bạn có nhiều nguồn với sitemap, có thể sẽ mất thời gian để enqueue hết, bạn có thể tạm thời disable sitemap trong sources.yaml để test nhanh)
