...
func (m *Model) PredictProba(x SparseVector) []float64 {
	z := make([]float64, m.K)
...
func (m *Model) SGDStep(x SparseVector, y int, lr float64) {
	p := m.PredictProba(x)
...
func (m *Model) TrainSGD(xs []SparseVector, ys []int, opt TrainOptions) {
	if opt.Epochs <= 0 {
		opt.Epochs = 5
	}
	if opt.LR <= 0 {
